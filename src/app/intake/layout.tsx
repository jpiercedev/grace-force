...
export const metadata: Metadata = {
  // Absolute, so the public form is not titled after the internal product.
  title: { absolute: 'Get in touch · Grace' },
  description: 'Send a message to the Grace team.',
}
...
export default function IntakeLayout({ children }: { children: ReactNode }) {
  return (
    <div className="flex min-h-dvh flex-col">
      <BrandBand label="Grace" tagline="We would love to hear from you" width="max-w-2xl" />
      ...
      <footer className="mx-auto w-full max-w-2xl px-4 pb-8 sm:px-6">
        <p className="border-t border-slate-200 pt-5 text-sm text-slate-600">
          Grace keeps what you send here only to reply to you.
        </p>
      </footer>
    </div>
  )
